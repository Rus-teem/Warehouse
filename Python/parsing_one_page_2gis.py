"""Парсинг одной страницы каталога дизайн-студий на 2GIS."""
...
url = "https://2gis.ru/kazan/search/%D0%94%D0%B7%D0%B0%D0%B9%D0%BD%20%D0%B8%D0%BD%D1%82%D0%B5%D1%80%D1%8C%D0%B5%D1%80%D0%BE%D0%B2/rubricId/331/page/5"


def get_studio_data(url):
    """Запрашивает страницу и извлекает данные о студиях."""
    response = requests.get(url, headers=headers)
    print(response)
    if response.status_code == 200:
        soup = BeautifulSoup(response.text, 'html.parser')
        studios = soup.find_all("div", class_="_1kf6gff")
        studio_data = []
        for studio in studios:
            name = studio.find("span", class_="_1cd6avd").text.strip() if studio.find("span", class_="_1cd6avd") else "Не указано"
